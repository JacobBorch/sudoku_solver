...
impl Solver for BruteforceSolver {
    fn solve(&self) -> Option<Grid> {
        Self::aux_solver(&mut self.grid.clone())
    }
}
...
impl BruteforceSolver {
    ...
    fn has_duplicate(row: &Vec<u8>) -> bool {
        ...
        row.iter().filter(|&&x| x != 0).collect::<HashSet<_>>().len() 
        != 
        row.iter().filter(|&&x| x != 0).collect::<Vec<_>>().len()
    }

    fn is_valid(grid: &Grid) -> bool {
        grid.iter().all(|row| !BruteforceSolver::has_duplicate(row))
            && (0..9)
                .map(|j| Self::extract_col(grid, j))
                .all(|col| !BruteforceSolver::has_duplicate(&col))
            && (0..9)
                .map(|j| Self::extract_square(grid, j))
                .all(|square| !BruteforceSolver::has_duplicate(&square))
    }
...
#[test]
fn test_next_entry() {
    let next_entry = BruteforceSolver::next_entry_to_change(&GRID);
...
fn has_duplicate_with_duplicate() {
    ...
    let has_duplicate = BruteforceSolver::has_duplicate(&row);
...
fn is_valid_when_valid() {
    let is_valid = BruteforceSolver::is_valid(&GRID);
    assert!(is_valid)
}

#[test]
fn invalid_row() {
    let is_valid = BruteforceSolver::is_valid(&INVALID_ROW_GRID);
    assert!(!is_valid)
}

#[test]
fn invalid_column() {
    let is_valid = BruteforceSolver::is_valid(&INVALID_COLUMN_GRID);
    assert!(!is_valid)
}

#[test]
fn invalid_square() {
    let is_valid = BruteforceSolver::is_valid(&INVALID_SQUARE_GRID);
    assert!(!is_valid)
}

#[test]
fn extract_col_works() {
    let seventh_col = BruteforceSolver::extract_col(&GRID, 6);
...
fn extract_square_works() {
    let sixth_square = BruteforceSolver::extract_square(&INVALID_SQUARE_GRID, 5);
...
#[test]
fn test_solve() {
    let solver = BruteforceSolver::new(SOLVABLE.clone());
    let solution = solver.solve();
    let is_valid_solution = BruteforceSolver::is_valid(&solution.unwrap());
